...
class TensorBoardWithLLStats(tf.keras.callbacks.TensorBoard):
  ...
  def __init__(self, eval_every, id_data, datasets, mode, normalize,visible_dist, **kwargs):
    ...
    for probs in probs_res:
      ...
      for dataset in self.datasets:
        if dataset == self.id_data:
          continue
        targets = np.concatenate([np.zeros_like(probs_res[probs][dataset]), np.ones_like(probs_res[probs][self.id_data])])
        lls = np.concatenate([probs_res[probs][dataset],probs_res[probs][self.id_data]])
        score_sum += sklearn.metrics.roc_auc_score(targets, lls)
      with summary_ops_v2.always_record_summaries():
        with self._val_writer.as_default():
          summary_ops_v2.scalar(f'{probs}_auroc_mean',score_sum/(len(self.datasets)-1),step=epoch)

  def on_epoch_end(self, epoch, logs=None):
    logging.info('Epoch %d completed', epoch)
    if epoch != 0 and epoch % self.eval_every == 0:
      probs_res = get_probs(self.datasets,self.model,self.mode,self.normalize,n_samples=10,split='val',visible_dist=self.visible_dist)
...
def get_probs(datasets,model,mode,normalize,n_samples,split='val',training=False,visible_dist='cont_bernoulli'):
